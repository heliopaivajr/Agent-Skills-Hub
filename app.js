...
function renderImport() {
  return `
    <div class="grid cols-2">
      <div class="card">
        <div class="section-title"><h2>Catálogo local _CLAUDE</h2><button class="primary" data-action="load-local-catalog">Carregar catálogo</button></div>
        <p class="muted">Carrega <code>data/local-library.json</code>, gerado a partir de <code>E:\\_HELIOJR\\_CLAUDE</code>, e separa arquivos por Skill, Agente, MCP ou Upload.</p>
        ${renderLocalCatalog()}
      </div>
      <div class="card">
        <h2>Importação JSON manual</h2>
        <p class="muted">Cole um pacote exportado ou um array de itens. A importação valida tipo, organização e duplicatas por slug.</p>
        <textarea id="import-json" placeholder='{"skills":[...]}'></textarea>
        <div class="toolbar"><button class="primary" data-action="import-json">Validar e importar</button></div>
      </div>
    </div>
...
function renderLocalCatalog() {
  if (!localCatalog) return `<div class="empty">Catálogo ainda não carregado. Rode por servidor local para permitir leitura do JSON.</div>`;
  const q = getParam("localQ");
  const kind = getParam("localKind");
  const filtered = localCatalogFiltered(q, kind);
  const preview = filtered.slice(0, 300);
  return `
    <div class="grid cols-4">
      ${["skill", "agent", "mcp", "upload"].map(type => `<div class="card metric"><span>${type}</span><strong>${localCatalog.counts?.[type] || 0}</strong></div>`).join("")}
    </div>
    <div class="toolbar">
      <input placeholder="Filtrar catálogo..." value="${escapeHtml(q)}" data-local-filter="localQ">
      <select data-local-filter="localKind">
        <option value="">Todos</option>
        ${["skill", "agent", "mcp", "upload"].map(type => `<option value="${type}" ${kind === type ? "selected" : ""}>${type}</option>`).join("")}
      </select>
      <button data-action="select-visible-catalog">Selecionar visíveis</button>
      <button class="primary" data-action="import-selected-catalog">Importar selecionados</button>
      <button data-action="import-filtered-catalog">Importar filtro atual</button>
    </div>
    <p class="muted">${filtered.length} itens encontrados. Mostrando ${preview.length}; importação do filtro atual é limitada a 500 itens por vez para manter o navegador leve.</p>
    <div class="catalog-list">
      ${preview.map(item => `
        <label class="catalog-row">
          <input type="checkbox" data-catalog-id="${item.id}">
          <span><strong>${escapeHtml(item.name)}</strong><br><small>${escapeHtml(item.kind)} · ${escapeHtml(item.relative_path)} · ${formatBytes(item.file_size)}</small></span>
        </label>
      `).join("") || `<div class="empty">Nenhum item no filtro atual.</div>`}
    </div>
  `;
}

function localCatalogFiltered(q = getParam("localQ"), kind = getParam("localKind")) {
  if (!localCatalog) return [];
  return localCatalog.items.filter(item => {
    const matchesKind = !kind || item.kind === kind;
    const haystack = `${item.name} ${item.file_name} ${item.relative_path} ${(item.tags || []).join(" ")} ${item.category}`.toLowerCase();
...
function bindPage() {
  ...
  const loadCatalog = document.querySelector("[data-action='load-local-catalog']");
  if (loadCatalog) loadCatalog.addEventListener("click", loadLocalCatalog);
  const selectVisible = document.querySelector("[data-action='select-visible-catalog']");
  if (selectVisible) selectVisible.addEventListener("click", () => document.querySelectorAll("[data-catalog-id]").forEach(input => input.checked = true));
  const importSelected = document.querySelector("[data-action='import-selected-catalog']");
  if (importSelected) importSelected.addEventListener("click", importSelectedCatalogItems);
  const importFiltered = document.querySelector("[data-action='import-filtered-catalog']");
  if (importFiltered) importFiltered.addEventListener("click", () => importCatalogItems(localCatalogFiltered().slice(0, 500)));
  document.querySelectorAll("[data-local-filter]").forEach(input => {
    input.addEventListener("input", () => setParam(input.dataset.localFilter, input.value));
    input.addEventListener("change", () => setParam(input.dataset.localFilter, input.value));
  });
...
async function loadLocalCatalog() {
  ...
  try {
    const response = await fetch("data/local-library.json", { cache: "no-store" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    localCatalog = await response.json();
    target.innerHTML = `<div class="card"><strong>Catálogo carregado:</strong> ${localCatalog.items.length} itens de ${escapeHtml(localCatalog.source_root)}.</div>`;
    render();
  } catch (error) {
    target.innerHTML = `<div class="notice">Não consegui carregar <code>data/local-library.json</code>. Rode com <code>python -m http.server 5173</code> e acesse pelo localhost. Detalhe: ${escapeHtml(error.message)}</div>`;
  }
}

function importSelectedCatalogItems() {
  if (!localCatalog) return;
  const ids = [...document.querySelectorAll("[data-catalog-id]:checked")].map(input => input.dataset.catalogId);
  const items = localCatalog.items.filter(item => ids.includes(item.id));
  importCatalogItems(items);
}

function importCatalogItems(items) {
  if (!items.length) return alert("Selecione pelo menos um item do catálogo.");
  const imported = [];
  items.forEach(source => {
    const uploadSlug = slugify(`${source.relative_path}`);
    let upload = orgItems("uploads").find(item => item.slug === uploadSlug);
    if (!upload) {
      upload = itemBase("upload", {
        name: source.file_name,
        slug: uploadSlug,
        file_name: source.file_name,
        original_file_name: source.file_name,
        file_type: source.extension,
        mime_type: mimeFromExtension(source.extension),
        file_size: source.file_size,
        storage_path: source.source_path,
        source_path: source.source_path,
        description: source.description,
        classification: source.kind === "skill" ? "Skill" : source.kind === "agent" ? "Agente" : source.kind === "mcp" ? "MCP" : "Arquivo de apoio",
    ...
    if (source.kind === "skill") imported.push(createFromCatalogSource("skills", source, upload, { content_markdown: source.content || `# ${source.name}\n\nOrigem: ${source.relative_path}` }));
    if (source.kind === "agent") imported.push(createFromCatalogSource("agents", source, upload, { role: source.name, system_prompt: source.content || `Você é ${source.name}.`, operational_instructions: `Origem: ${source.relative_path}` }));
    if (source.kind === "mcp") imported.push(createFromCatalogSource("mcps", source, upload, { config_json: source.content || JSON.stringify({ name: source.name, source_path: source.source_path }, null, 2), security_level: "médio" }));
    if (source.kind === "upload") imported.push(`Upload: ${source.name}`);
  });
  state.import_logs.push({ id: uid("imp"), organization_id: state.currentOrgId, summary: imported, source: "local-library", created_at: today(), created_by: state.currentUserId });
  saveState();
  render();
  const result = document.getElementById("import-result");
  if (result) result.innerHTML = `<div class="card"><strong>${imported.length} registro(s) importados do catálogo local.</strong><div class="list">${imported.slice(0, 30).map(x => `<div>${escapeHtml(x)}</div>`).join("")}${imported.length > 30 ? `<div class="muted">...e mais ${imported.length - 30}</div>` : ""}</div></div>`;
}

function createFromCatalogSource(key, source, upload, extra) {
  const exists = orgItems(key).find(item => item.slug === slugify(source.relative_path));
  if (exists) return `${ENTITY_META[key].singular}: ${exists.name} (já existia)`;
  const item = itemBase(ENTITY_META[key].type, {
    name: source.name,
    slug: slugify(source.relative_path),
    description: source.description,
  ...
  state[key].push(item);
  syncFileLinks(key, item.id, [upload.id], source.kind === "mcp" ? "Configuração" : "Documento principal");
  return `${ENTITY_META[key].singular}: ${item.name}`;
}

function mimeFromExtension(ext) {
  const map = { md: "text/markdown", txt: "text/plain", json: "application/json", yaml: "application/yaml", yml: "application/yaml", pdf: "application/pdf", docx: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", html: "text/html", py: "text/x-python" };
